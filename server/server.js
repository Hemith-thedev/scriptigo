...
const MONGO_URI = process.env.MONGO_URI || "mongodb://127.0.0.1:27017/scriptigo";
...
app.post("/api/tags", async (req, res) => {
  ...
  if (!name) return res.status(400).json({ status: "error", message: "Tag name is required" });
  ...
  try {
    ...
    if (error.code === 11000) return res.status(409).json({ status: "error", message: "Tag name already exists" });
...
app.post("/api/scripts/:id/tags", async (req, res) => {
  ...
  if (!Array.isArray(tag_ids)) return res.status(400).json({ status: "error", message: "Tag IDs must be an array" });

  try {
    const updatedLine = await Script.findByIdAndUpdate(script_id, { tags: tag_ids }, { new: true });
    if (!updatedLine) return res.status(404).json({ status: "error", message: "Script line not found" });
...
app.put("/api/tags/:id", async (req, res) => {
  ...
  try {
    const result = await Tag.findByIdAndUpdate(id, { name: name.trim(), color: color || "#FFD700" }, { new: true });
    if (!result) return res.status(404).json({ status: "error", message: "Tag not found" });
...
app.post("/api/genres", async (req, res) => {
  ...
  try {
    ...
    await newGenre.save();
    res.status(201).json({ status: "success", message: "Genre created successfully", id: newGenre._id });
  } catch (error) {
    if (error.code === 11000) return res.status(409).json({ status: "error", message: "Genre already exists" });
...
app.put("/api/genres/:id", async (req, res) => {
  ...
  try {
    const result = await Genre.findByIdAndUpdate(id, { name: name.trim() }, { new: true });
    if (!result) return res.status(404).json({ status: "error", message: "Genre not found" });
...
app.post("/api/stories", async (req, res) => {
  ...
  if (!title) return res.status(400).json({ status: "error", message: "Title and valid Genre array are required" });

  try {
    const newStory = new Story({ title: title.trim(), genres });
    await newStory.save();
    res.status(201).json({ status: "success", message: "Story created successfully", id: newStory._id });
  } catch (error) {
...
app.put("/api/stories/:id", async (req, res) => {
  ...
  try {
    const result = await Story.findByIdAndUpdate(id, { title: title.trim(), genres }, { new: true });
    if (!result) return res.status(404).json({ status: "error", message: "Story not found" });
...
app.delete("/api/stories/:id", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ status: "success", message: "Story and related assets deleted successfully" });
  } catch (error) {
    return res.status(500).json({ status: "error", message: "Could not delete story" });
  }
});
...
app.post("/api/scripts", async (req, res) => {
  ...
  try {
    const newLine = new Script({
      story_id, order_id, type, speaker_name, emotion, action, vocal,
...
app.get("/api/stories/:story_id/scripts", async (req, res) => {
  ...
  try {
    ...
    const rows = await Script.find(filters).populate('tags').sort({ order_id: 1 });
    res.status(200).json({ status: "success", data: rows });
  } catch (error) {
    res.status(500).json({ status: "error", message: "Error compiling script elements mapping" });
  }
});
...
app.put("/api/scripts/:id", async (req, res) => {
  ...
  try {
    const result = await Script.findByIdAndUpdate(id, {
      order_id, speaker_name, emotion, action, vocal, is_hidden, is_important
    }, { new: true });

    if (!result) return res.status(404).json({ status: "error", message: "Script line not found" });
...
app.delete("/api/scripts/:id", async (req, res) => {
  ...
  try {
    ...
    if (!result) return res.status(404).json({ status: "error", message: "Line not found" });
...
app.post("/api/exports", async (req, res) => {
  ...
  try {
    const newExport = new Export({ story_id, file_name, format: format || "pdf" });
    await newExport.save();
    res.status(201).json({ status: "success", message: "Export history recorded", export_id: newExport._id });
  } catch (error) {
...
app.put("/api/exports/:id", async (req, res) => {
  ...
  if (!new_file_name || new_file_name.trim() === "") return res.status(400).json({ status: "error", message: "Valid new file name required" });

  try {
    const currentExport = await Export.findById(id);
    if (!currentExport) return res.status(404).json({ status: "error", message: "Export record not found" });
    ...
    const result = await Export.findByIdAndUpdate(id, { file_name: sanitizedName }, { new: true });
...
app.post("/api/stories/:story_id/versions", async (req, res) => {
  ...
  if (!version_name || !script_data) return res.status(400).json({ status: "error", message: "Required dataset properties missing" });

  try {
    const newVersion = new ScriptVersion({ story_id, version_name: version_name.trim(), script_data });
    await newVersion.save();
    res.status(201).json({ status: "success", version_id: newVersion._id, message: "Script version snapshot archived safely! 🛡️" });
  } catch (error) {
...
app.get("/api/stories/:story_id/versions", async (req, res) => {
  const { story_id } = req.params;
  try {
    const rows = await ScriptVersion.find({ story_id }, 'version_name createdAt').sort({ createdAt: -1 });
    res.status(200).json({ status: "success", data: rows });
  } catch (error) {
